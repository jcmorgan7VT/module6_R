...
**Answer Q.1**
Uncertainty is any source of error in the final output made by a forecasting model, and they arise from input data, input drivers, and model formulation.
...
**Answer Q.2**
Knowing the uncertainty makes forecasts actionable for resource managers. For example, the low uncertainty prediction of an algal bloom would be much more useful for the manager to advise folks to not use the lake.
...
**Answer Q.4**
Increasing water temperature can result in decreased dissolved oxygen, negatively impacting aquatic species. Additionally, it can increase algae concentrations.
...
**Answer Q.5**
Yes, air temperature and water temperature appear to be highly coupled.
...
**Answer Q.6**
The multiple grey lines represents the different ensemble members of the air temperature prediction from NOAA.
...
**Answer Q.7**
The multiple linear regression uses the water temp yesterday and the air temperature today to predict the water temp today, altered by a y intercept and coefficients.
...
**Answer Q.8**
The water temperature yesterday was a much more important predictor or water temp today, because the coefficient is larger. The standard error for the coefficients on the inputs are low, but the y intercept standard error is much higher.
...
**Answer Q.9** 
The r2 is high, and the err and rmse are low, indicating that the model performs well. A qualitative assessment of the predicted versus actual values shows a strong model fit.
...
**Answer Q.11** 
The forecasted water temperature is very similar to the forecasted air temperature, probably because the air temperature is the only input to the linear model other than the water temperature the previous day, so air temperature over time will drive the model, while on a day to day basis the water temperature the day before is more important.
...
**Answer Q.12**
Forecast uncertainty grows larger and larger the further out from observations one goes.
...
**Answer Q.13** 
The probablistic forecast represents uncertainty, which makes me more hesistate to believe that the temperature would be what the deterministic predticts.
...
**Answer Q.14**
Parameter uncertainty can be reduced by using more observations to calibrate parameters, by changing a parameter to result in model outputs that better match observed data.
...
**Answer Q.15**
Initial condition uncertainty occurs due to a lack of measurement of initial conditions, or to account for the lack of precision in initial conditions measurements.
...
forecast_total_unc <-
  ...
  rows_update(ic_df,
              by = c("forecast_date", "ensemble_member", "forecast_variable")) 
```
...
```{r}
#add this forloop to my model forecast
for(i in 2:length(forecasted_dates)) {
...
**Answer Q.16**
Parameter uncertainty contributes the most uncertainty beyond the first day of the forecast.
...
**Answer Q.17**
Different water temperature forecast models would have different quantities and distriubtions of uncertainty because they would be parameterized different ways, probably built on different data sets and initial conditions which controls how much uncertainty from each of those sources occur.
